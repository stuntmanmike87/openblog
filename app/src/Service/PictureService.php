...
final class PictureService
{
    ...
    public function square(UploadedFile $picture, ?string $folder = '', ?int $width = 250): string
    {
        ...
        switch ($pictureInfos['mime']) {
            case 'image/png':
                /** @var \GdImage $sourcePicture */
                $sourcePicture = imagecreatefrompng((string) $picture);
                break;
            case 'image/jpeg':
                /** @var \GdImage $sourcePicture */
                $sourcePicture = imagecreatefromjpeg((string) $picture);
                break;
            case 'image/webp':
                /** @var \GdImage $sourcePicture */
                $sourcePicture = imagecreatefromwebp((string) $picture);
                break;
            default:
                throw new \Exception('Format d\'image incorrect');
        }
